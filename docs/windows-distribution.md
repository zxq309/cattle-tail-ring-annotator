...
## 当前版本 3.3.0（2026-09-10）

普通用户下载 Release 的完整 Setup.exe；Git 源码不包含运行库、模型权重或数据。软件自带离线运行组件，启动更新检查仍需联网；已打开的工程可离线继续标注。见 [客户端更新](client-updates.md)、[3.3 发布说明](release-330.md) 和 [功能实测](release-330-validation.md)。

本版增加数据整理、七类分组和逐记录设备身份，修复旧工程索引与全流程衔接。关于/帮助中的图文操作说明书覆盖准备、整理、标注、导出和常用维护。以下 r3/r2 条目是历史记录。
