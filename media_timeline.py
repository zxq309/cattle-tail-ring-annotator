...
MIN_ISOLATED_SEGMENT_MS = 5_000.0
...
def build_timeline_index(
    ...
    for segment_index, (raw_start_ms, raw_end_ms) in enumerate(raw_segments):
        raw_segment_duration_ms = max(
            frame_duration_ms, raw_end_ms - raw_start_ms
        )
        # A very short fragment after a discontinuity is usually a corrupt
        # recorder tail. Seeking into it can crash VLC's MPEG-PS decoder, so
        # keep its raw bounds for diagnostics but do not expose it as an
        # annotatable part of the public timeline.
        duration_ms = (
            0.0
            if segment_index > 0
