...
> **3.2.1 修订（2026-09-09，同版本更新）：** 所有九轴记录的横轴、悬停和当前位置统一显示工程已有同步关系下的日期时间，可输入时间戳定位。启动先检查更新，有新版自动下载安装，完成重开后才能标注；直接更新到最新版，跳过中间版本。失败时可重试或退出。已装早前 3.2.1 的用户请从现有发布页手动下载修订包；本次不递增版本号。
